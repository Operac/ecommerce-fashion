...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch products
      try {
        const productsRes = await fetch('http://localhost:5000/product/getAllProducts', {
      ...
      // Fetch categories
      try {
        const categoriesRes = await fetch('http://localhost:5000/category/getAllCategories', {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleDeleteProduct = async (productName) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/product/deleteProduct/${productName}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleSaveEditProduct = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/product/updateProduct/${editingProduct.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        setProducts(products.map(p => p.id === editingProduct.id ? { ...p, ...editFormData } : p));
          ...
          <div className="bg-white p-6 rounded border border-gray-300">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-black">{formatPrice(totalRevenue)}</p>
              </div>
              <FaChartBar className="text-black text-4xl" />
            </div>
          </div>
          <div className="bg-white p-6 rounded border border-gray-300">
            <div className="flex items-center justify-between">
              ...
              {filteredProducts.length > 0 ? (
                filteredProducts.map(product => (
                  <tr key={product.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-6 py-3 text-sm text-gray-600 truncate">{product.description}</td>
